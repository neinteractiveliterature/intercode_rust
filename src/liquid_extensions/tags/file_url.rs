...
use crate::{
  active_storage_attachments, active_storage_blobs, cms_files,
...
impl FileUrlTag {
  pub fn new(cms_parent: Option<CmsParent>, db: Arc<DatabaseConnection>) -> Self {
      ...
      active_storage_attachments::Entity::find()
        ...
        .filter(
          active_storage_attachments::Column::RecordId.in_subquery(
            QuerySelect::query(&mut self.cms_parent.cms_files().column(cms_files::Column::Id))
              .to_owned(),
          ),
        )
        .one(self.db.as_ref()),
